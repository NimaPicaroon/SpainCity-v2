...
function GetSkills()
	local tiempo = 1000
	local minutos = tiempo * 60
	local espera = minutos * 20
	local player = W.GetPlayerData()

	yourStats = player.stats
	Citizen.SetTimeout(espera, function()
		GetSkills()
	end)
end
...
local Extras = {
	["Top"] = {
		...
		Table = {
			...
			Extra = { 
						--{Drawable = 11, Id = 15, Tex = 0, Name = "Extra Shirt"},
...
function ToggleClothing(which, extra)
	...
	if which ~= "Mask" then
		if not Gender then W.Notify('NOTIFICACION', 'Este modelo de personaje no permite esta opción', 'error') return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		W.Notify('NOTIFICACION', 'No parece haber ninguna variante para esto','error') return
	else
		if not LastEquipped[which] then
				...
				PlayToggleEmote(Toggle.Emote, function()
					...
					if Toggle.Table.Extra then
						...
						for k,v in pairs(Extras) do
								--if (yourStats.strength and yourStats.stamina and tonumber(yourStats.strength) + tonumber(yourStats.stamina)) > 175 then
	...
	W.Notify('NOTIFICACION', 'Ya estás usando esto','error') return false
end
...
function ToggleProps(which)
	...
	if not Prop.Variants then
		...
		W.Notify('NOTIFICACION', 'No parece que tengas nada que quitarte', 'error') return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then W.Notify('NOTIFICACION', 'Este modelo de personaje no permite esta opción', 'error') return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		W.Notify('NOTIFICACION', 'No parece haber ninguna variante para esto', 'error') return false
	end
end
...
RegisterCommand("pelo", function ()
	local Emote = { Dict = "clothingtie", Anim = "check_out_a", Move = 51, Dur = 1000 }
	if lastHair then
		SetPedComponentVariation(PlayerPedId(), 2, lastHair, 0, 0)
		PlayToggleEmote(Emote, function()
			SetPedComponentVariation(PlayerPedId(), 2, lastHair, 0, 0)
		end)
		lastHair = nil
	else
