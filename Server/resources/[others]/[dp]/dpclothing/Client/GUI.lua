...
local function HoveredButton()
	...
	if Distance < 0.015 then
		Text(Config.GUI.Position.x, Config.GUI.Position.y-0.09, 0.3, 'Info', false, false, true)
	end
end
...
local function DrawGUI()
		...
		if Button and not Cooldown then	-- If the button is clicked we execute the command, just like if the player typed it in chat.
			if v.Sprite == "gloves" then
				if not LastEquipped["Shirt"] then
					...
					W.Notify('ERROR', "No puedes hacer esto sin tu camisa", 'error')
				end
			else
				PushedButton(k)  ExecuteCommand(v.Command)
			end
	...
	if Cooldown then Text(x, y+0.05, 0.28, "Por favor, espera...", false, false, true) end 		-- Cooldown indicator, if theres a cooldown we display a little text.
	...
	if InfoButton then 															
		W.Notify('ERROR', "Si el botón es azul, tienes un elemento guardado")
	end
end

local TextureDicts = {"dp_clothing", "dp_wheel"}
local opened = false
RegisterKeyMapping('clothes', 'Abrir menú de la ropa', 'keyboard', 'K')
