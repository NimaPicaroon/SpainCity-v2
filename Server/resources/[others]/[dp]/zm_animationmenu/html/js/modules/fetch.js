...
export const createPanels = (panelData) => {
    ...
    panelData.forEach(panel => {
        if (panel && panel.type) {
            ...
            block.id = (panel.dances && panel.dances.dict) || (panel.scenarios && panel.scenarios.scene) || (panel.expressions && panel.expressions.expression) || (panel.walks && panel.walks.style) || (panel.animals && panel.animals.dict) || (panel.plus_eighteen && panel.plus_eighteen.dict) || (panel.stance && panel.stance.dict)
            ...
            block.setAttribute('data-stance', (panel.stance) ? (JSON.stringify({dict: panel.stance.dict, anim: panel.stance.anim, duration: panel.stance.duration, prop: panel.stance.prop, propBone: panel.stance.propBone, propPlacement: panel.stance.propPlacement})) : false);
            ...
            block.setAttribute('data-plus_eighteen', (panel.plus_eighteen) ? (JSON.stringify({dict: panel.plus_eighteen.dict, anim: panel.plus_eighteen.anim, duration: panel.plus_eighteen.duration})) : false);
            ...
            block.setAttribute('data-animals', (panel.animals) ? (JSON.stringify({dict: panel.animals.dict, anim: panel.animals.anim, duration: panel.animals.duration})) : false);            
            ...
            block.addEventListener('click', e => {
                if (e.target.textContent != 'bookmark_add') {
                    fetchNUI('beginAnimation', {dance: JSON.parse(block.getAttribute('data-dances')), stance: JSON.parse(block.getAttribute('data-stance')), scene: JSON.parse(block.getAttribute('data-scenarios')), expression: JSON.parse(block.getAttribute('data-expressions')), plus_eighteen: JSON.parse(block.getAttribute('data-plus_eighteen')), walk: JSON.parse(block.getAttribute('data-walks')), prop: JSON.parse(block.getAttribute('data-props')), particle: JSON.parse(block.getAttribute('data-particles')), shared: JSON.parse(block.getAttribute('data-shared')), animals: JSON.parse(block.getAttribute('data-animals')), disableMovement: JSON.parse(block.getAttribute('data-disableMovement')), disableLoop: JSON.parse(block.getAttribute('data-disableLoop'))}).then((resp) => {
                        (resp.e)
