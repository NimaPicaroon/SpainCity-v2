...
-- Script starts here
local helicam = false
local polmav_hash = GetHashKey("h135")
local geo_hash = GetHashKey("littlebirdgeo")
local eheli_hash = GetHashKey("eheli")
...
function IsPlayerInPolmav()
	...
	local can  = false
	if IsVehicleModel(vehicle, polmav_hash) or IsVehicleModel(vehicle, eheli_hash) or IsVehicleModel(vehicle, geo_hash) then
		can = true
	end
	return can
end
