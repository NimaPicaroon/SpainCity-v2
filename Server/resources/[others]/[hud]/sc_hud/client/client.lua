CreateThread(
	function()
		...
		SendNUIMessage({ action = 'setLogo', value = "https://i.imgur.com/yQudmOZ.png" })
...
Citizen.CreateThread(
	function()
		while true do
			...
			if OwnData.job then
				local job = OwnData.job.label .. ': ' .. OwnData.job.ranklabel
				local blackMoney = OwnData.money.coins
				local bank = OwnData.money.bank
				local money = OwnData.money.money
				...
				if OwnData.job.rankname == "boss" then
					SendNUIMessage({ action = 'setMoney', id = 'society', value = exports["Ox-Jobcreator"]:getJobData(OwnData.job.name) })
				else
