...
window.onload = function () {
		...
		var eventCallback = {
			ui: function(data) {
				...
				if (config.showVoice == true) { document.querySelector('#voice').style.display = 'block'; $("#voice-text").text("Normal"); }
				else if (config.showVoice == false) { document.querySelector('.info.time-and-place').style.right = '25px';}
				if (config.showVoiceText == true) { document.querySelector('#voice-text').style.display = 'block'; }
			...
			setVoiceDistance: function(data) {
				...
				var voicetext = document.querySelector('#voice-text').innerHTML;
				...
				switch(data.value){
					case 'shout':
						$("#voice-text").text("Gritar");
						break;
					case 'whisper':
						$("#voice-text").text("Susurrar");
						break;
					default:
						$("#voice-text").text("Normal");
						break;
				}
			},
