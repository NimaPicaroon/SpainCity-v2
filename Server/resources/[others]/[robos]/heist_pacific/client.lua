...
Citizen.CreateThread(function()
    for k, v in pairs(Config['PacificHeist']['startHeist']['peds']) do
        ...
        DecorSetInt(PacificHeist['startPeds'][k], 'SPAWNEDPED', 1)
...
function StartHeist()
    W.TriggerCallback('Wave:GetPlayersJob', function(players)
        if players and #players >= 1 then
            W.TriggerCallback('pacificheist:server:checkTime', function(time)
                if time then
                    ...
                    TriggerServerEvent('ZC-Dispatch:sendAlert', 'police', '¡El Banco Pacific está siendo robado, necesitamos refuerzos!', GetEntityCoords(PlayerPedId()), GetPlayerServerId(PlayerId()), 'heist', 'pacificbank')
                end
            end)
        end
    end, 'police', true)
...
-- RegisterCommand('start', StartHeist)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if startDist <= 2.0 and AVAILABLE_WEAPONS[GetSelectedPedWeapon(ped)] and not PacificHeist['start'] then
            sleep = 1
            inZone = true

            W.ShowText(Config['PacificHeist']['startHeist']['pos'], '~y~Pacific~w~\nRobar', 0.6, 8)
        ...
        if inZone and not shown then
            ...
            exports['ZC-HelpNotify']:open('Usa <strong>E</strong> para interactuar', 'action_two_pacificbank')
        elseif not inZone and shown then
            shown = false

            exports['ZC-HelpNotify']:close('action_two_pacificbank')
        end
    ...
    Citizen.CreateThread(function()
        ...
        while vaultLoop do
            ...
            if inZone and not shown then
                ...
                exports['ZC-HelpNotify']:open('Usa <strong>E</strong> para interactuar', 'grab_money_pacificbank')
            elseif not inZone and shown then
                shown = false
    
                exports['ZC-HelpNotify']:close('grab_money_pacificbank')
            end
    ...
    Citizen.CreateThread(function()
        while extendedLoop do
            ...
            if inZone and not shown then
                ...
                exports['ZC-HelpNotify']:open('Usa <strong>E</strong> para interactuar', 'action_pacific_bank')
            elseif not inZone and shown then
                shown = false

                exports['ZC-HelpNotify']:close('action_pacific_bank')
            end
...
function LaptopHack(index)
    W.TriggerCallback('ZCore:checkItem', function(hasItem, itemData)
        if hasItem then
    ...
    end, Config['PacificHeist']['requiredItems'][5])
end
...
function PaintingScene(sceneId)
    ...
    if weapon ~= GetHashKey('WEAPON_SWITCHBLADE') then W.Notify('Banco', '¿Cómo pretendes cortar este cuadro?', 'error') return end
    ...
    repeat
        if not shown then
            shown = true
            exports['ZC-HelpNotify']:open('Usa <strong>FLECHA</strong> para cortar', 'cut_paint_pacific')
        end

        if IsControlJustPressed(0, 175) then
            scenes[1] = true
            shown = false
            exports['ZC-HelpNotify']:close('cut_paint_pacific')
        end
        Wait(1)
    until scenes[1] == true
...
function PlantingAnim(type, index, pos, rot)
    ...
    W.TriggerCallback('ZCore:checkItem', function(hasItem, itemData)
        if hasItem then
            exports['ZC-HelpNotify']:close('action_two_pacificbank')
            busy = true
            TriggerServerEvent('pacificheist:server:removeItem', Config['PacificHeist']['requiredItems'][itemIndex], itemData.slotId)
            local ped = PlayerPedId()
            local pedCo, pedRotation = GetEntityCoords(ped), GetEntityRotation(ped)
                        ...
                        repeat
                            ...
                            if not nearCellGates() then
                                ...
                                if IsControlJustPressed(0, 246) then
                                    ...
                                    exports['ZC-HelpNotify']:close('detonate_pacific_bank')
                                    remote = {}
                                    pressed = true
                                    remoteLoop = false
                                end
                                ...
                                if inZone and not shown then
                                    ...
                                    exports['ZC-HelpNotify']:open('Usa <strong>Y</strong> para detonar', 'detonate_pacific_bank')
                                elseif not inZone and shown then
                                    shown = false
...
function Hacking(index)
    W.TriggerCallback('ZCore:checkItem', function(hasItem, itemData)
        if hasItem then
            ...
            TriggerEvent("mhacking:start", 7, 60, function(outcome)
