        ...
        while HeistModule._data.stealing do
            ...
            if HEIST_DATA.shops[HeistModule._data.closest] then
                ...
                if DoesEntityExist(info.data.entity) then
                    ...
                    if dist > 10.0 and GlobalState.HeistShops[HeistModule._data.closest].data.robbing then
                        exports.progressbar:Cancel()
                        HeistModule._data.stealing = false
                        TriggerServerEvent('shopheist:endedRobByDistance', HeistModule._data.closest)
                        Wave.Notify('Tendero', 'Te has alejado mucho del robo y lo has cancelado', 'info')
                    end
                end
            end
...
---Reset store
RegisterNetEvent('shopheist:resetStore', function(index)
    ...
    if data then
        ...
        local cashRegister = GetClosestObjectOfType(entityCoords, 5.0, `prop_till_01_dam`)
        local cashCoords = GetEntityCoords(cashRegister)

        if DoesEntityExist(cashRegister) then
            CreateModelSwap(cashCoords, 0.5, `prop_till_01_dam`, `prop_till_01`, false)
        end
...
RegisterNetEvent('shopheist:endPed', function(index)
    if not index or not HEIST_DATA.shops[index] then
        return W.Notify('Tendero', 'Ha fallado algo, contacta con un desarrollador.', 'warn')
    end
    ...
    if HeistModule._data.stealing then
        ...
        local entityCoords = GetEntityCoords(data.data.entity)
        local cashRegister = GetClosestObjectOfType(entityCoords, 5.0, `prop_till_01`)
        local cashCoords = GetEntityCoords(cashRegister)
        local cashRotation = GetEntityRotation(cashCoords)
        ...
        if not IsPedDeadOrDying(data.data.entity) then
            ...
            CreateModelSwap(cashCoords, 0.5, `prop_till_01`, `prop_till_01_dam`, false)
                ...
                while DoesEntityExist(bag) do
                    ...
                    if playerPed and playerCoords then
                        ...
                        if dist <= 2.5 then
                            ...
                            TriggerServerEvent('shopheist:addMoney', index, secret)
                            DeleteEntity(bag)
                ...
                if DoesEntityExist(data.data.entity) then
                    ...
                    if #(playerPos - entityCoords) <= 10.0 then
                        ...
                        if (IsPedArmed(playerPed, 1) or IsPedArmed(playerPed, 4)) and IsPlayerFreeAimingAtEntity(playerId, data.data.entity) then
                            if (not GlobalState.HeistShops[HeistModule._data.closest].data.robbed) and (not GlobalState.HeistShops[HeistModule._data.closest].data.robbing) then
                                if not check then
                                    check = true
                                    ...
                                    Wave.TriggerCallback('Wave:GetPlayersJob', function(cops)
                                        if cops and #cops >= HEIST_DATA.cops then
                                            ...
                                            HeistModule:check()
                                            TriggerServerEvent('shopheist:startRobbing', HeistModule._data.closest)
                                            Wait(500)
                                            ...
                                            TriggerServerEvent('ZC-Dispatch:sendAlert', 'police', '¡Un badulaque está siendo robado, necesitamos refuerzos!', playerPos, GetPlayerServerId(playerId), 'heist', 'shop')
                                            ...
                                            {}, {}, {}, function() 
                                                ...
                                                if #(playerPos - entityCoords) <= 12.5 then
                                                    TriggerServerEvent('shopheist:endedRob', HeistModule._data.closest)
                                                end
                                            end)
                                        else
                                            Wave.Notify('Tendero', 'Lo siento, no me queda nada para darte, no me hagas nada porfavor.', 'info')
                                        end
                                    end, 'police', true)
                                end
                                ...
                                if not notify then
                                    notify = true
                                    Wave.Notify('Tendero', '¿Qué quieres robarme? Ya lo han hecho hace un rato...', 'info')
                                end
