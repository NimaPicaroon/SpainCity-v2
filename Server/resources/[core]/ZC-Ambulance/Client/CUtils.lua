...
function secondsToClock(seconds)
	...
	if seconds <= 0 then
		...
		return tonumber(mins), tonumber(secs)
	end
end
...
function GetCauseOfDeath()
	local death = GetPedCauseOfDeath(PlayerPedId())
	local deathString = "Unknown"

	if WeaponsHash[tostring(death)] ~= nil then
		deathString = WeaponsHash[tostring(death)]
	end
