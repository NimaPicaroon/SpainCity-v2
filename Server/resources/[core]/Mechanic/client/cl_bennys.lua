...
local function saveVehicle()
    ...
    local myCar = W.GetVehicleProperties(veh)
    local plate = GetVehicleNumberPlateText(veh)
    TriggerServerEvent('qb-customs:server:updateVehicle', myCar, textparse(plate))
end
...
local function AllowJob(restrictionData, job)
    ...
    print('Denied for not having allowed job. (' .. job .. ')')
    return false
end
...
local coloursJSON = json.decode(LoadResourceFile('Mechanic', 'colours.json'))
...
function PreviewColour(paintType, paintCategory, paintID)
    ...
    for key, value in next, coloursJSON do
        if tonumber(value.ID) == paintID then
            rgbColour = tostring(value.RGB)
        end
    end

    for token in (rgbColour..","):gmatch("(.-),%s*") do
        finalColour[i] = token
        i = i + 1
    end 
    ...
    if paintType == 0 then --Primary Colour
        if paintCategory == 1 then --Metallic Paint
            SetVehicleCustomPrimaryColour(plyVeh, tonumber(finalColour[1]), tonumber(finalColour[2]), tonumber(finalColour[3]))
            SetVehicleExtraColours(plyVeh, originalPearlescentColour, originalWheelColour)
        else
            SetVehicleCustomPrimaryColour(plyVeh, tonumber(finalColour[1]), tonumber(finalColour[2]), tonumber(finalColour[3]))
        end
    elseif paintType == 1 then --Secondary Colour
        SetVehicleCustomSecondaryColour(plyVeh, tonumber(finalColour[1]), tonumber(finalColour[2]), tonumber(finalColour[3]))
...
function ExitBennys()
    ...
    if next(CustomsData) then
        SetupInteraction(CustomsData.location)
    end
...
function EnterLocation(override)
    ...
    if not canEnter then
        W.Notify('Tuneos', 'You cant do anything here!')
        ExitBennys()
        return
    end
...
function GetLocations()
    W.TriggerCallback("qb-customs:server:GetLocations", function(locations)
        ...
        CreateThread(function()
            while next(CustomsData) and not isPlyInBennys do
                if not shown then
                    ...
                    exports['ZC-HelpNotify']:open('Usa <strong>E</strong> para tunear', 'mechanic_interact')
                end

                if IsControlJustReleased(0, 38) and CheckRestrictions(CustomsData.location) then
                    EnterLocation()
                    return
                end
...
function CheckForGhostVehicle()
    ...
    local closestVehicle, closestDistance = W.GetClosestVehicle(GetEntityCoords(PlayerPedId()))
...
function CheckRestrictions(location)
    ...
    return isEnabled and DoesEntityExist(vehicle) and allowedJob
end

function SetupInteraction(location)
    local text = CustomsData.drawtextui
    local _location = Config.Locations[location]

    if _location.restrictions then
        local Job = W.GetPlayerData().job

        if AllowJob(_location.restrictions, Job.name) then
            text = '[E] ' .. text
            CheckForKeypress()
        end
    else
...
-- Location Creation
CreateThread(function()
    while not PlayerData or not PlayerData.job do Wait(2500) end
        ...
        for i, spot in ipairs(data.zones) do
            ...
            newSpot:onPlayerInOut(function(isPointInside, _)
                ...
                elseif CustomsData['location'] == location and CustomsData['spot'] == _name then
                    ...
                    exports['ZC-HelpNotify']:close('mechanic_interact')
                end
            end)
        end
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() and W.GetPlayerData() ~= {} then
        GetLocations()
    end
end)
...
RegisterNetEvent("qb-customs:client:purchaseSuccessful", function()
    ...
    W.Notify('Tuneos', "Purchase Successful")
end)

RegisterNetEvent("qb-customs:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    W.Notify('Tuneos', "Not enough money", "error")
end)
...
RegisterNetEvent('qb-customs:client:EnterCustoms', function(override)
    ...
    print(CheckRestrictions(CustomsData.location))
