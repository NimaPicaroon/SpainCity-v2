...
$(function(){
    window.addEventListener("message", function(event){
        ...
        if (event.data.democolorbar) {
            ...
            const htmlString = `
            <div class="democontainer" style="background: rgb(${event.data.democolor}); background-image: url('${event.data.picBack}'); background-position: center; background-repeat: no-repeat; background-size: 300px auto;">
...
function notif(queued) {
    ...
    const element = document.getElementById("all")
    const htmlString = `
    <div class="container" style="background: rgb(${queued.color}); background-image: url('${queued.picBack}'); background-position: center; background-repeat: no-repeat; background-size: 300px auto;">
