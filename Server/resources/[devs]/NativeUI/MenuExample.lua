...
mainMenu = UIMenu.New("Native UI", "~y~NATIVEUI SHOWCASE")
...
function AddMenuKetchup(menu)
    ...
    menu.OnCheckboxChange = function(sender, item, checked_)
        if item == newitem then
            ketchup = checked_
            ShowNotification("~r~Ketchup status: ~y~"..tostring(ketchup))
        end
    end
end
...
function AddMenuFoods(menu)
    ...
    menu.OnListChange = function(sender, item, index)
        if item == newitem then
            ...
            ShowNotification("Preparing ~y~" .. dish .. "~w~...")
        end
    end
end
...
function AddMenuFoodCount(menu)
    ...
    menu.OnSliderChange = function(sender, item, index)
        if item == newitem then
            ...
            ShowNotification("Preparing ~r~" .. quantity .. " ~y~" .. dish .. "(s)~w~...")
        end
    end
end
...
function AddMenuCook(menu)
    ...
    menu.OnItemSelect = function(sender, item, index)
        if item == newitem then
            local string = "You have ordered ~r~" .. quantity .. " ~y~"..dish.."(s)~w~ ~r~with~w~ ketchup."
            if not ketchup then
                string = "You have ordered ~r~" .. quantity .. " ~y~"..dish.."(s)~w~ ~r~without~w~ ketchup."
            end
