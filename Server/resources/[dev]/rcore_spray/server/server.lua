...
AddEventHandler('rcore_spray:addSpray', function(spray)
    ...
    local have, sprayitem = Player.getItem("spraycan")
    
    if have then
        Player.removeItemFromInventory("spraycan", 1, sprayitem.slotId)
        local i = 1
        while true do
        ...
        W.SendToDiscord("graffiti", "NUEVO GRAFFITI", spray.text, source)
    else
