...
window.addEventListener('message', function(event) {
	...
	switch (event.data.action) {
		case 'openCraft':
			...
			for(var i = 0; i < num; i++) {
				...
				row += `
					<div class="col-md-3">
						<div class="card item-card" id="${itemId}wb_${event.data.name}">
							<div class="card-body item-card-body d-flex justify-content-center align-items-center" style="background-color: rgba(220, 220, 220, 0.20)" id="selected${itemId}wb_${event.data.name}">
								<span class="item-title text-center">${itemName[itemId].label}</span>
								<img src="./images/${itemName[itemId].name}.png" class="image">
							</div>
						</div>
					</div>
				`;
				var myEle = document.getElementById(itemId+"wb_"+event.data.name);
				if(!myEle) {
					$(document).on('click', "#"+itemId+"wb_"+event.data.name, function() {
						allID = this.id;
						...
						sound.volume = 0.3;
						sound.play();
						$.post('https://Ox-Crafting/action', JSON.stringify({
							action: "craft",
							item: id,
							crafts: craft,
							itemName: itemName,
	...
	case 'openSideCraft':
		...
		var recipe = ``;
		var img = `
			<img src="./images/${event.data.itemNameID}.png" class="image_itemselected">
			...
			if (event.data.recipe[i][2]){
				recipe += `
					<div class="d-flex align-items-center mx-1">
						<img src="./images/${event.data.recipe[i][0]}.png" class="image_components">
						<span style="color: #fff; font-weight: 600; font-size: 20px;" class="ms-2">${event.data.itemNames[idName].label} x${event.data.recipe[i][1]} (${event.data.inventory[i].key})</span>
					</div>
				`;
			} else {
				recipe += `
